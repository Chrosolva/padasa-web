    ...
    <section class="content">
        <div class="panel">
            <div class="panel-body">
                <form role="form" class="form-inline mb-2" method="GET" action="{{ url('/dashboard/lhpexecutive/lhpRestanPanenBJR') }}">
                    <div class="row">
                            ...
                            <table id="table-data" class="table table-bordered table-striped table-hover datatablefilter" >
                                <thead>
                                    <tr>
                                        ...
                                        <th style="font-size: 12px;" data-toggle="tooltip" data-placement="left" title="Produksi BJR (KG) = Janjang Produksi * BJR">TOTAL KG BJR</th>
                                        <th style="font-size: 12px;">KG R0 (KG)</th>
                                        <th style="font-size: 12px;">KG R1 (KG)</th>
                                        <th style="font-size: 12px;">KG R2 (KG)</th>
                                        <th style="font-size: 12px;">KG R3 (KG)</th>
                                ...
                                <tbody>
                                    @foreach ($lhp_RestanPanenBJR as $row)
                                        <tr>
                                            ...
                                            <td>{{$row->KEBUN}}</td>
                                            <td style="text-align: right;">{{number_format($row->TOTAL_KG_BJR_BKM,0,',','.')}}</td>
                                            @if($row->SITE_ID < 3200 && ($row->KG_R0 > ($row->TOTAL_KG_BJR_BKM * 0.02)))
                                                <td class="bg-red" style="text-align: right;">{{number_format($row->KG_R0,0,',','.')}}</td>
                                            @elseif ($row->SITE_ID >= 3200 && ($row->KG_R0 > (0.05 * $row->TOTAL_KG_BJR_BKM)))
                                                <td class="bg-red" style="text-align: right;">{{number_format($row->KG_R0,0,',','.')}}</td>
                                            @else 
                                                <td style="text-align: right;">{{number_format($row->KG_R0,0,',','.')}}</td>
                                            @endif
                                            <td style="text-align: right;">{{number_format($row->KG_R1,0,',','.')}}</td>
                                            <td style="text-align: right;">{{number_format($row->KG_R2,0,',','.')}}</td>
                                            <td style="text-align: right;">{{number_format($row->KG_R3,0,',','.')}}</td>
