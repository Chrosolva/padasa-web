    ...
    <section class="content">
        <div class="panel">
            <div class="panel-body">
                <form role="form" class="form-inline mb-2" method="GET" action="{{ url('/dashboard/lhpexecutive/lhpRestanPanen') }}">
                    <div class="row">
                    ...
                    <div class="box-body">
                        <div class="box-body table-responsive">
                            <table id="table-data" class="table table-bordered table-striped table-hover datatable" >
                                <thead>
                                    <tr>
                                        <th style="font-size: 12px;">TGL ANGKUT</th>
                                        <th style="font-size: 12px;">TBS PRODUKSI</th>
                                        <th style="font-size: 12px; display: none;">R0</th>
                                ...
                                <tbody>
                                    @foreach ($lhp_RestanPanen as $row)
                                        <tr>
                                            <?php 
                                                $date = date_create($row->TGLANGKUT);
                                            ?>
                                            <td>{{date_format($date, 'd/m/y')}}</td>
                                            <td>{{number_format($row->TBSPRODUKSI,0,',','.')}}</td>
                                            <td style="display: none;">{{number_format($row->R0,0,',','.')}}</td>
                                            <td>{{number_format($row->R1,0,',','.')}}</td>
                                            <td>{{number_format($row->R2,0,',','.')}}</td>
                                            @if($row->R3 > 0) 
                                                <td class="bg-red">{{number_format($row->R3,0,',','.')}}</td>
                                            @else
                                                <td>{{number_format($row->R3,0,',','.')}}</td>
                                            @endif
                                            @if($row->R4 > 0) 
                                                <td class="bg-red">{{number_format($row->R4,0,',','.')}}</td>
                                            @else
                                                <td>{{number_format($row->R4,0,',','.')}}</td>
                                            @endif
                                            @if($row->R5 > 0) 
                                            <td class="bg-red">{{number_format($row->R5,0,',','.')}}</td>
                                        @else
                                            <td>{{number_format($row->R5,0,',','.')}}</td>
                                        @endif
                                        </tr>
                                    @endforeach
                                </tbody>
...
@section('script-content')
    <script type="text/javascript">
        ...
        makeDataTableResponsiveAutoWidth('table-data', 0, 'asc', -1);
