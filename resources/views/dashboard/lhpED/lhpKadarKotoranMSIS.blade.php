...
@section('header-title')
    KADAR KOTORAN  {{ $jenis === 'MS' ? 'Minyak Sawit (MS)' : 'Inti Sawit (IS)' }} – PMKS
@endsection
...
<section class="content">
    <div class="panel">
        <div class="panel-body">
            <form role="form" class="form-inline mb-2" method="GET" action="{{ url('/dashboard/lhpexecutive/KadarKotoranMSIS') }}">
                <div class="row">
                            ...
                            <thead>
                                @if ($jenis === 'MS')
                                    <tr>
                                        ...
                                        <th style="font-size:12px;">KADAR KOTORAN T1 (%)</th>
                                        <th style="font-size:12px;">VOL. T1 (KG)</th>
                                        <th style="font-size:12px;">KADAR KOTORAN T2 (%)</th>
                                        <th style="font-size:12px;">VOL. T2 (KG)</th>
                                        <th style="font-size:12px;">KADAR KOTORAN T3 (%)</th>
                                        <th style="font-size:12px;">VOL. T3 (KG)</th>
                                        <th style="font-size:12px;">KADAR KOTORAN T4 (%)</th>
                                        <th style="font-size:12px;">VOL. T4 (KG)</th>
                                        <th style="font-size:12px;">KADAR KOTORAN TPL1 (%)</th>
                                        <th style="font-size:12px;">VOL. TPL1 (KG)</th>
                                        <th style="font-size:12px;">KADAR KOTORAN TPL2 (%)</th>
                                        <th style="font-size:12px;">VOL. TPL2 (KG)</th>
                                        <th style="font-size:12px;">KADAR KOTORAN PROD (%)</th>
                                ...
                                @foreach ($rows as $row)
                                    <tr>
                                        @php
                                            ...
                                            // handle 'YYYY-MM-DD...' or other raw text
                                            $tglObj = $tglRaw ? \DateTime::createFromFormat('Y-m-d', substr($tglRaw,0,10)) : null;
                                        ...
                                        <td>{{ $tglObj ? $tglObj->format('d/m/Y') : ($tglRaw ?? '-') }}</td>
                                        <td>{{ $row->KEBUN ?? '-' }}</td>

                                        @if ($jenis === 'MS')
                                            {!! $fmt($row->KADAR_KOTORAN_T1 ?? null) !!}
                                            {!! $fmtInt($row->VOLUME_T1 ?? null) !!}

                                            {!! $fmt($row->KADAR_KOTORAN_T2 ?? null) !!}
                                            {!! $fmtInt($row->VOLUME_T2 ?? null) !!}

                                            {!! $fmt($row->KADAR_KOTORAN_T3 ?? null) !!}
                                            {!! $fmtInt($row->VOLUME_T3 ?? null) !!}

                                            {!! $fmt($row->KADAR_KOTORAN_T4 ?? null) !!}
                                            {!! $fmtInt($row->VOLUME_T4 ?? null) !!}

                                            {!! $fmt($row->KADAR_KOTORAN_TPL1 ?? null) !!}
                                            {!! $fmtInt($row->VOLUME_TPL1 ?? null) !!}

                                            {!! $fmt($row->KADAR_KOTORAN_TPL2 ?? null) !!}
                                            {!! $fmtInt($row->VOLUME_TPL2 ?? null) !!}

                                            {!! $fmt($row->KADAR_KOTORAN_PRODUKSI ?? null) !!}
