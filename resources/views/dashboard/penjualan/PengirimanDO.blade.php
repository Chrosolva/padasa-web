    ...
    <section class="content">
        <div class="panel">
            <div class="panel-body">
                <form role="form" class="form-inline" method="GET" action="{{ url('/dashboard/penjualan/pengiriman-DO') }}">
                        ...
                        <table id="table-data" class="table table-bordered table-striped table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th>PRODUK</th>
                                    <th>QTY DO</th>
                                    <th>QTY PENGIRIMAN SBLM</th>
                                    <th>QTY PENGIRIMAN</th>
                                    <th>SISA</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($pengiriman_DO as $row)
                                    <tr>
                                        <?php 
                                            $date = date_create($row->TGL_DO);
                                        ?>
                                        <td>{{$row->PABRIK}}</td>
                                        <td>{{$row->NODO}}</td>
                                        ...
                                        <td>{{$row->PRODUK}}</td>
                                        <td style="text-align: right;">{{number_format($row->QTY_DO,0,',','.')}}</td>
                                        <td style="text-align: right;">{{number_format($row->QTY_PENGIRIMAN_SBLM,0,',','.')}}</td>
                                        <td style="text-align: right;">{{number_format($row->QTY_PENGIRIMAN_SELECTED_DATE,0,',','.')}}</td>
